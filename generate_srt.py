#!/usr/bin/env python3
"""
カット調整済み動画からSRT字幕ファイルを生成するスクリプト

Usage:
    python generate_srt.py <video_path> [options]

このスクリプトは、EDLでカット編集し、Premiereで微調整した後の
動画ファイルから字幕（SRT）ファイルを生成します。
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='カット調整済み動画からSRT字幕ファイルを生成'
    )
    ...
    try:
        ...
        # 2. 音声を文字起こし
        print("📝 音声を文字起こし中...")
        transcriber = Transcriber(api_key, WHISPER_API_ENDPOINT, WHISPER_MODEL)
        ...
        # 整形済みテロップデータを作成
        captions_path = temp_dir / f"{video_path.stem}_captions.json"
