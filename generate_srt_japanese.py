#!/usr/bin/env python3
"""
カット調整済み動画からSRT字幕ファイルを生成するスクリプト（日本語特化版）
文字レベルのタイムスタンプに対応

Usage:
    python generate_srt_japanese.py <video_path> [options]
"""
...
def main():
    parser = argparse.ArgumentParser(
        description='カット調整済み動画からSRT字幕ファイルを生成（日本語特化版）'
    )
    ...
    if args.output:
        ...
        output_path = output_dir / f"{video_path.stem}_subtitles_japanese.srt"
    ...
    try:
        ...
        if 'words' in transcript:
            print(f"✅ 文字タイムスタンプを取得しました（{len(transcript['words'])}文字）")
            # 最初の数文字を表示
            print("📍 サンプル: ", end="")
            ...
            print("⚠️ 警告: 文字タイムスタンプが利用できません")
        
        # 3. キャプションをフォーマット（日本語版）
        print("✂️ キャプションをフォーマット中（日本語最適化）...")
        ...
        # 整形済みテロップデータを作成（日本語用タイミング照合）
        captions_path = temp_dir / f"{video_path.stem}_captions_japanese.json"
