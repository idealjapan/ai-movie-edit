...
class Transcriber:
    ...
    def transcribe(self, audio_path):
        ...
        data = {
            "model": self.model,
            "response_format": "verbose_json",  # タイムスタンプ情報を含む詳細な結果を取得
            "language": "ja"  # 日本語を指定（日本語動画の場合）
        }
        ...
        try:
            ...
            with open(audio_path, "rb") as audio_file:
                ...
                transcript_path = os.path.join(output_dir, f"{Path(audio_path).stem}_transcript.json")
                with open(transcript_path, "w", encoding="utf-8") as f:
                    json.dump(result, f, ensure_ascii=False, indent=2)
                
                print(f"文字起こしが完了しました。結果を保存しました: {transcript_path}")
                return result
